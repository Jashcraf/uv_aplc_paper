...
\affil[f]{Muon Space, Mountain View, CA, 94043, USA}
...
\section{Is Throughput a function of Bandwidth?}
\label{sec:bandwidth_survey}
One avenue for maximizing the ``throughput" of UV APLCs is to increase the bandwidth of the coronagraph. While coronagraphs require modest bandwidths of $\approx 10-20\%$ because of chromatic wavefront error, in principle, a UV APLC could be optimized to support a larger bandwidth if the wavefront sensing and control can support it. We begin answering this question with another APLC design survey, where the core throughput is evaluated in $2\%$ increases of bandwidth beginning at $4\%$ and ending at $50\%$. A minimum of 5 wavelengths are used to sample the bandwidth for every design, with more wavelengths added so that there is not a greater than $2\%$ change between adjacent wavelengths. In other words, all designs with a bandwidth of $\leq 10\%$ use 5 wavelengths across the bandwidth, and each step beyond uses an additional wavelength for each $2\%$ step in bandwidth.

\begin{figure}
    \centering
    \includegraphics[width=0.5\textwidth]{bw_survey.png}
    \caption{Survey of the core throughput at $10 \lambda_C/D_C$ of APLCs at different bandwidths. Beginning at $2\%$ and ending at $50\%$, spaced in $2\%$ increments. We find that at an inner working angle of $6 \lambda/D$, the core throughput is not a strong function of the bandwidth. Only at the $48\%$ and $50\%$ solutions did the solver fail to find an acceptable solution. These solutions were grayscale, and had a precipitous drop in throughput. However, these data suggest that a $46\%$ bandwidth UV APLC will only suffer from a $\approx 5\%$ decrease in core throughput, and more than make up for this loss with $4.6\times$ more photons than the point design. This of course neglects the important problem of wavefront sensing and control in such a wide bandwidth, which was not considered for these calculations. We only suggest that the APLC design is not bandwidth-limited, and this can be a degree of freedom for future investigators to use to maximize throughput.}
    \label{fig:bw_survey}
\end{figure}

What we find is a modest decrease in core throughput as a function of bandwidth. The point design's $\approx 60\%$ core throughput can be marginally improved by going to a shorter bandwidth, at the cost of substantially reducing the number of photons. Alternatively, the number of photons let through the filter can be substantially increased with a marginal decrease in core throughput. As an example, we observe that moving from a bandwidth of $10\%$ to $20\%$ results in a $\approx3\%$ degradation in core throughput, but double the number of photons. Critically, this analysis neglects wavefront sensing and control (WFSC), which is particularly challenging in the ultraviolet. The WFSC abilities of a UV coronagraph will be the ultimate limiter to bandwidth, but the results presented here suggest that it is not a substantive limiter to the nominal design of the coronagraph. Therefore, the total system throughput can \emph{in principle} be improved by going to larger bandwidths. Future work should consider the optimal bandwidth for photometric characterization in the UV, and the limitations of WFSC in the UV to further refine this analysis. 

Important to consider when changing the design of the APLC is how the response to low-order aberrations changes. The increased IWA offers us robustness to small amplitudes of aberration, but given that a constant wavefront error budget results in twice the effective aberration as in the visible (i.e., $500nm$ of OPD is  $\approx1$ wave of error in the visible, but $\approx2$ waves of error in the UV), the sensitivity of a UV coronagraph onboard HWO is expected to be comparable to the visible\cite{roser_uv_2026}. However, an increased bandwidth effectively ``decreases" the inner working angle of the longest wavelength in the bandwidth, which could in turn decrease its sensitivity to low-order aberrations at that wavelength. To evaluate the low-order aberration sensitivity of these coronagraphs we follow the procedure used to analyze the LUVOIR coronagraphs in Juanola-Parramon 2022\cite{Roser_2022}. A log-scan of 3rd-order aberration amplitudes are injected into the entrance pupil of the coronagraph and the corresponding contrast degradation is observed at the focal plane. This is repeated for aberrations spanning the entire entrance pupil (pupil-common), as well for low-order aberrations that vary segment-to-segment (segment-to-segment) to understand the sensitivity of each mode to these aberrations. From the log-scan of aberration amplitudes, we compute the slope of the contrast degradation around $4\times 10^{-11}$ contrast to get the final sensitivity. The final contrast is then multiplied by $10^{12}$, and the wavefront error scaled to picometers ($pm$) to express the final sensitivity in pico-part per picometer. The results of this sensitivity analysis are shown in Figure \ref{fig:sensitivities_bandwidth}, where the aberration sensitivities appear uncorrellated to the bandwidth.

\begin{figure}
    \centering
    \includegraphics[width=\linewidth]{sensitivities_bandwidth.png}
    \caption{Caption}
    \label{fig:sensitivities_bandwidth}
\end{figure}

\section{Is throughput a function of Lyot Stop geometry?}
\label{sec:ls_survey}
The next design study of interest was to determine if throughput could be gained going to alternative Lyot stop geometries. In addition to the circular lyot stops used in the point design, we explore Lyot stops with eroded segment edges (left, Figure \ref{fig:lyot_geometries}), and a shrunk variant of the entrance pupil's aperture edges (right, Figure \ref{fig:lyot_geometries}). We generate a different Lyot stop in $1\%$ increments from a ratio of the Lyot stop diameter to the circumscribed entrance pupil diameter, $D_{LS}/D_{EP}$, for the circular lyot stop. The degree of degradation for the eroded segmented and eroded edges are then generated by what degree of erosion yields the same integrated energy through the Lyot mask.

\begin{figure}
    \centering
    \includegraphics[width=0.7\linewidth]{lyot_geometries.png}
    \caption{Caption}
    \label{fig:lyot_geometries}
\end{figure}

The result of this design survey is given in Figure \ref{fig:lyot_study}, which shows that the eroded segment designs yield much lower throughput solutions than the circular aperture designs, while the eroded aperture edges produce nearly the same core throughput. This study suggests that the point design may have had a Lyot stop that was too far open, given that the throughput increasing roughly linearly until an $85\%$ diameter, where it then tapers off. Only $1-3\%$ core throughput is lost by reducing the Lyot diameter to $85\%$ of the circumscribed diameter, and it could grant lower alignment sensitivity. From an alignment perspective the circular Lyot stop is the most optimal. The rotational degree of freedom means that the clocking of the Lyot mask about the propagation direction is invariant. As before, we repeat the sensitivity analysis for the eroded segment designs (Figure \ref{fig:sensitivity_segments}), and eroded edge designs (Figure \ref{fig:sensitivity_edges}).

\begin{figure}
    \centering
    \includegraphics[width=0.8\linewidth]{ls_survey.png}
    \caption{Caption}
    \label{fig:lyot_study}
\end{figure}

\begin{figure}
    \centering
    \includegraphics[width=\linewidth]{sensitivities_lyotstop_segments.png}
    \caption{Caption}
    \label{fig:sensitivity_segments}
\end{figure}

\begin{figure}
    \centering
    \includegraphics[width=\linewidth]{sensitivities_lyotstop_edges.png}
    \caption{Caption}
    \label{fig:sensitivity_edges}
\end{figure}

\section{Conclusions}
\label{sec:conclusions}
The real brr was the friends we made along the way
